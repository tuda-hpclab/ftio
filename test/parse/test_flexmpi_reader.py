...
def test_bandwidth_is_iobytes_over_iotime():
    data, ranks = extract(_msg(iobytes=4000, rtime=10.0, iotime=0.4), [])

    assert ranks == 0
    bw = _bandwidth(data)
    assert bw["b_rank_avr"] == [pytest.approx(10000.0)]  # iobytes / iotime
...
def test_zero_iotime_is_a_zero_width_zero_bandwidth_sample():
    data, _ = extract(_msg(iobytes=4000, rtime=7.0, iotime=0.0), [])
    bw = _bandwidth(data)
...
def test_end_to_end_prediction():
    ...
    for i in range(40):
        ...
        iobytes = 5.0e8 if iotime else 0.0
        msgs.append(_msg(iter=i, iobytes=iobytes, rtime=float(i) + iotime, iotime=iotime))
